...
class _StationsMapState extends State<StationsMap> {
  ...
  Future<Marker> Function(Cluster<ClusterItem>)? get _markerBuilder => (Cluster<ClusterItem> cluster) async {
        final Iterable<Station> stationCluster = cluster.items.cast<Station>();
        return Marker(
            markerId: MarkerId(cluster.getId()),
            position: cluster.location,
            onTap: () {
              context.read<StationsBloc>().add(StationClusterTappedEvent(cluster: cluster));
            },
  ...
  Color _getColorFromAvgAvailability(Iterable<Station> items) {
    ...
    switch (availabilityMap.entries.reduce((a, b) => a.value > b.value ? a : b).key) {
      case 'busy':
        return busyColor;
      case 'available':
